...
object MHello {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Infinite loop..
    // Keep server running
    eventLoop(selector, server)
  }

  def eventLoop(selector: Selector, server: ServerSocketChannel) = {
    while ({ true }) {
      log(
      ...
      val crunchifyIterator =
        crunchifyKeys.iterator
      while ({ crunchifyIterator.hasNext }) {
        val myKey = crunchifyIterator.next
        // Tests whether this key's channel is ready to accept a new socket connection
        if (myKey.isAcceptable) {
          val crunchifyClient = server.accept
          ...
          httpResponse(crunchifyClient)("Hello World\n")
